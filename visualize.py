...
if __name__ == '__main__':
    cnt = 10
    df = pd.read_csv(r"C:\Public\git\_github\GTF-Paper\versionsWithVersion.txt", sep=";")
    df = df[df["Dependency"].str.contains(r"android|org\.jetbrains") == False]
    df = df.head(cnt).sort_values('Count', ascending=True)
